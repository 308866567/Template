...
public class Dialog_SetPoint extends Dialog {
	 ...
	 * @param style
	 */
	public Dialog_SetPoint(Shell parent, int style) {
		super(parent, style);
		setText("SWT Dialog");
	}
	...
	protected void createContents() {
		...
		composite_left.addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				...
				Label t = new Label(composite_left, SWT.NONE);
				...
				if (start != null) {
					// TODO 上一次选中的点恢复颜色
					start.setImage(SWTResourceManager.getImage(SetPoint2.class, "/image/dibiao2.png"));
					SwtUtils.autoImage(start);
				}
				// 选中的点会变色
				start = t;
				t.setImage(SWTResourceManager.getImage(SetPoint2.class, "/image/dibiao.png"));
				SwtUtils.autoImage(t);
				// 左键进入该点的设置面板
	...
	void initMap(Composite composite_left) {
		...
		// 设置背景图片
				SwtUtils.autoImage2(composite_left, "/image/map.jpg");
...
//		control.setBackground(SWTResourceManager.getColor(SWT.COLOR_WIDGET_FOREGROUND));
		...
		control.setImage(SWTResourceManager.getImage(SetPoint2.class, "/image/dibiao2.png"));
		...
		control.addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				...
				start = control;
				// TODO
				control.setImage(SWTResourceManager.getImage(SetPoint2.class, "/image/dibiao.png"));
				SwtUtils.autoImage(control);
				// 左键进入该点的设置面板
				// 传入选中点的id,id=0时数据库只进行添加操作
